...
---
## 1 补丁图像 (Patch images)
- 已经修改好-王涛 ```tissue_mask.py tumor_mask.py non_tumor_mask.py```
...
2.2.1 为了训练WSI分类器，首先需要将所有的训练集通过CNN模型得到概率热力图谱，

```extract_feature_probsmap.py```

2.2.2 然后在概率热力图中使用以下命令提取的特征：
...
#---------------以上代码调通，下面测试因为没有测试集的tifmask，无法定位预测的评价-------------
